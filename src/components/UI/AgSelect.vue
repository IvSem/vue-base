...
<template>
	...
	>
		<div class="current" @click="onWrapperClick">
			<span :class="{ 'is-selected': !!state.selectedOption }" @click="onWrapperClick">{{ selectedText }}</span
		...
		<div class="list-wrapper" :class="{ 'show-up': state.openAbove }">
			<div v-if="props.search" class="search">
				<input :id="state.searchId" placeholder="Search" type="text" @input="onSearchInput" @blur="onEscape" />
			</div>
